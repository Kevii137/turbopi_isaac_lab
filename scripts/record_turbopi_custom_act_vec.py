...
parser.add_argument("--dataset_name", type=str, default="turbopi_custom_act_cvae")
...
from common import CAMERA_LINK_TO_SENSOR_POS, CAMERA_LINK_TO_SENSOR_ROT, build_turbopi_cfg, get_wheel_joint_ids, twist_to_wheel_targets
from custom_turbopi_act_scene import CustomACTSceneCfg, route_waypoints
...
def repeated_waypoints(task_name: str) -> tuple[tuple[float, float], ...]:
    base = route_waypoints(CustomACTSceneCfg(), task_name)
    points = list(base)
...
def build_scene_cfg_class(num_envs: int):
    ...
    road_mat = sim_utils.PreviewSurfaceCfg(diffuse_color=(0.07, 0.07, 0.08), roughness=0.96)
    shoulder_mat = sim_utils.PreviewSurfaceCfg(diffuse_color=(0.22, 0.22, 0.22), roughness=0.98)
    floor_mat = sim_utils.PreviewSurfaceCfg(diffuse_color=(0.16, 0.16, 0.16), roughness=0.98)
    ...
    _Cfg = configclass(type("_CustomActVecSceneCfg", (InteractiveSceneCfg,), attrs))
    return _Cfg
...
def main() -> None:
    ...
    writer = ACTMountainSessionWriter(
        ...
        session_name=args_cli.session_name or datetime.utcnow().strftime("custom_act_vec_%Y%m%d_%H%M%S"),
