"""Keyboard teleoperation for TurboPi on the custom ACT layout."""
...
parser = argparse.ArgumentParser(description="Drive TurboPi with keyboard teleoperation on the custom ACT layout.")
...
MOUNT_CAMERA_POS = (0.140, 0.0, 0.115)
MOUNT_CAMERA_ROT = (0.987688, 0.0, -0.156434, 0.0)
...
def reset_to_custom_start(robot, scene_cfg: CustomACTSceneCfg) -> None:
    start_position, start_yaw = start_pose(scene_cfg)
    reset_robot_pose(robot, position=start_position, yaw=start_yaw)


def main() -> None:
    sim_cfg = sim_utils.SimulationCfg(dt=1.0 / 120.0, render_interval=1, device=args_cli.device, gravity=(0.0, 0.0, -9.81))
    ...
    set_robot_camera_mount(MOUNT_CAMERA_POS, MOUNT_CAMERA_ROT)

    sim.reset()
    reset_to_custom_start(robot, scene_cfg)
    sim.play()
    ...
    def reset_cb() -> None:
        ...
        if omega_tracker is not None:
            omega_tracker.reset()
        print("[INFO] Robot reset to custom ACT layout start.")

    def zero_command_cb() -> None:
        filtered_command.zero_()
        print("[INFO] Command zeroed.")
    ...
    teleop.add_callback("L", zero_command_cb)
    teleop.add_callback("ESCAPE", close_cb)
    teleop.reset()

    print(f"[INFO] TurboPi USD : {resolve_asset_usd(args_cli.asset_usd) if args_cli.asset_usd else 'default asset'}")
    print("[INFO] Scene       : custom ACT layout")
    ...
    while simulation_app.is_running():
        ...
        robot.set_joint_velocity_target(twist_to_wheel_targets(applied_command, robot.device), joint_ids=wheel_joint_ids)
