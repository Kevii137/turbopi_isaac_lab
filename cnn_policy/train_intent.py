"""Train the task-conditioned TurboPi cliff CNN policy."""
...
from . import DEFAULT_FRAME_HISTORY, DEFAULT_IMAGE_HEIGHT, DEFAULT_IMAGE_WIDTH, REPO_ROOT
from .intent_dataset import build_datasets
from .intent_model import IntentCNNConfig, build_model, save_checkpoint

DEFAULT_INTENT_DATA_ROOT = str(REPO_ROOT / "data" / "cnn_cliff_intent")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train the TurboPi task-conditioned cliff CNN policy")
    parser.add_argument("--episodes-dir", default=DEFAULT_INTENT_DATA_ROOT)
    parser.add_argument("--run-dir", default="runs/cliff_intent_cnn_v1")
    ...
    parser.add_argument("--preload-threshold-records", type=int, default=128)
    parser.add_argument("--preload-threshold-frames", type=int, default=250_000)
    parser.add_argument("--no-progress", action="store_true")
    parser.add_argument("--no-augment", action="store_true")
    parser.add_argument("--uniform-sampler", action="store_true")
    return parser
    ...
    *,
    ...
    if train_dataset.records and (
        ...
        print(
            ...
            f"(~{estimated_gb:.2f} GB resized frames)."
        )
        train_dataset.preload_all(show_progress=show_progress, desc="Decode train episodes", workers=preload_workers)
        if len(val_dataset.records) > 0:
            val_dataset.preload_all(show_progress=show_progress, desc="Decode val episodes", workers=preload_workers)
...
def evaluate_model_with_progress(
    ...
    *,
    ...
    bar = tqdm(total=len(loader.dataset), desc=f"Epoch {epoch:03d}/{epochs:03d} val", unit="sample", dynamic_ncols=True, leave=False)
    try:
        for batch in loader:
            images = batch["image"].to(device)
            task_ids = batch["task_index"].to(device)
            ...
            bar.set_postfix(avg_loss=f"{total_loss / total_examples:.4f}", samples_per_s=f"{total_examples / elapsed:.1f}")
    finally:
        bar.close()
...
def train_epoch(
    ...
    *,
    ...
    bar = None
    if show_progress:
        bar = tqdm(total=len(loader.dataset), desc=f"Epoch {epoch:03d}/{epochs:03d} train", unit="sample", dynamic_ncols=True, leave=False)
    ...
    try:
        for batch in loader:
            ...
            if bar is not None:
                elapsed = max(time.perf_counter() - start_time, 1e-6)
                bar.update(batch_size)
                bar.set_postfix(avg_loss=f"{total_loss / total_examples:.4f}", lr=f"{lr:.2e}", samples_per_s=f"{total_examples / elapsed:.1f}")
    finally:
        if bar is not None:
            bar.close()
...
def main() -> None:
    ...
    print(f"[train] Task vocabulary: {task_names}")

    history: list[dict[str, float]] = []
    best_metric = float("inf")
    best_epoch = -1
    interrupted = False
    task_to_index = train_loader.dataset.task_to_index
    ...
    try:
        for epoch in range(1, args.epochs + 1):
            ...
            save_checkpoint(checkpoint_dir / "last.pt", model, epoch=epoch, metrics=record, extra=checkpoint_extra)

            metric = record["val_loss"] if not math.isnan(record["val_loss"]) else record["train_loss"]
            if metric < best_metric:
                best_metric = metric
                best_epoch = epoch
                save_checkpoint(checkpoint_dir / "best.pt", model, epoch=epoch, metrics=record, extra=checkpoint_extra)
